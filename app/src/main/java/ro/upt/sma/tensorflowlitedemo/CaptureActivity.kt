    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cmv_camera_preview.setOnClickListener { _ ->
            GlobalScope.launch((Dispatchers.Default)) {
                ...
                val recognitions = classifier.recognize(resizedBitmap)

                withContext(Dispatchers.Main) {
                    // TODO 3: Show the recognitions using the common Toast widget. Make use of joinToString method to concat multiple items.

                    val recognitionString  = recognitions.joinToString(separator = "\n") {
                        "Label: " + it.label + " " + "Confidence: " + it.confidence }

                    Toast.makeText(applicationContext, recognitionString, Toast.LENGTH_LONG).show()
                }
            }
        }
